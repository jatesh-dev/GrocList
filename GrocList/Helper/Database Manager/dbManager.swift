...
class DbManager {
	...
	func observer(event: DataEventType, chatKey: String, completion: @escaping (Result<[Messages], Error>) -> Void) {
		self.dbReference.child("ChatRoom").child(chatKey).observe(event, with: { (_) in
			self.dbReference.child("ChatRoom").child(chatKey).child("messages").getData { (error, snapMessages) in
	...
	func getData(chatKey: String, completion: @escaping (Result<[Messages], Error>) -> Void) {
		self.dbReference.child("ChatRoom").child(chatKey).child("messages").getData {(_, snapMessages) in
	...
	func sendMessage(chatKey: String, values: [String: Any]) {
		dbReference.child("ChatRoom").child(chatKey).child("messages").childByAutoId().updateChildValues(values as [AnyHashable: Any]) { (_, dbReference) in
			dbReference.child("ChatRoom").child(chatKey).observe(.childChanged, with: { (_) in
			})
		}
	}
